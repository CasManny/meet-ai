...
export const UpgradeView = () => {
  ...
  const { data: currentSubcription } = useSuspenseQuery(
      ...
      <div className="mt-4 flex-1 flex flex-col gap-y-10 items-center">
        <h5 className="font-medium text-2xl md:text-3xl">
          ...
          <span className="font-semibold text-primary">
            {currentSubcription?.name ?? "Free"}
          </span>
          plan
        </h5>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {products.map((product) => {
            const isCurrentProduct = currentSubcription?.id === product.id;
            const isPremium = !!currentSubcription;
